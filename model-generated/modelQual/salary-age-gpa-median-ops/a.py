...
# Replace the numeric columns with the median values from the new columns
numeric_columns = ['GPA', 'Age', 'Salary']
for col in numeric_columns:
    df[col] = df[f'{col}_median']

# Drop the '_median' columns
df.drop(columns=[f'{col}_median' for col in numeric_columns], inplace=True)

# Drop duplicate rows
df.drop_duplicates(inplace=True)

# Test the output
assert len(df) == len(df['Name'].unique()), "Duplicate rows were not dropped"
for col in numeric_columns:
    assert (df[col] == df.groupby('Name')[col].transform('median')).all(), f"Median values were not correctly filled for column {col}"
print(df)