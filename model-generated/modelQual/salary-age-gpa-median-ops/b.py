...
df = df.drop([f'{col}_median' for col in numeric_columns], axis=1)
...
# Test the output
assert len(df) == len(df['Name'].unique()), "Duplicate rows exist"
assert (df[numeric_columns] == df.groupby('Name')[numeric_columns].transform('median')).all().all(), "Median values not correctly assigned"